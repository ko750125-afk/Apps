    ...
    {displayImage && !hasError ? (
      <>
        ...
        <Image 
          ...
          unoptimized
        />
        <div className="absolute inset-0 bg-gradient-to-tr from-white/10 to-transparent pointer-events-none opacity-50 z-20" />
      </>
    ) : (
...
export default function AppCard({ app, index }: AppCardProps) {
  ...
  const wrapperClass = cn(
    "group relative h-full flex flex-col overflow-hidden rounded-[32px] transition-all duration-500",
    "bg-[#f0f0f3] border border-white/40",
    "shadow-[6px_6px_12px_#d1d1d6,-6px_-6px_12px_#ffffff]",
    "hover:shadow-[8px_8px_16px_#d1d1d6,-8px_-8px_16px_#ffffff,0_10px_20px_rgba(0,0,0,0.05)]",
  ...
  return (
    ...
    >
      <MotionLink
        ...
        whileTap={{ scale: 0.98 }}
      >
        <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-30" />
        {cardContent}
      </MotionLink>
    </motion.div>
  );
